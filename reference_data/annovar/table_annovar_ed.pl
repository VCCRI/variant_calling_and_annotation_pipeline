#!/usr/bin/env perl
# VCCRI - mictro copied changes by eddip (EI) from previous version of annovar
# Modifications will be marked with "#EI
use warnings;
use strict;
use Pod::Usage;
use Getopt::Long;
...
our $REVISION = '$Revision: 9f9e0f9efe83690a15a6aeb7714f1fc3a2341deb $';
our $DATE =	'$Date: 2018-04-16 00:47:49 -0400 (Mon, 16 Apr 2018) $';  
our $AUTHOR =	'$Author: Kai Wang <kaichop@gmail.com> $';

our ($verbose, $help, $man);
our ($queryfile, $dbloc);
our ($outfile, $buildver, $remove, $checkfile, $protocol, $operation, $otherinfo, $onetranscript, $nastring, $genericdbfile, $gff3dbfile, $bedfile, $vcfdbfile, $csvout, $argument, $tempdir, $vcfinput, $dot2underline, $thread, $maxgenethread, $polishgene, $xreffile, $convertarg, $codingarg);
our $orig_command = $0;
...
GetOptions('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 'outfile=s'=>\$outfile, 'buildver=s'=>\$buildver, 'remove'=>\$remove, 'checkfile!'=>\$checkfile, 
	...
	'thread=i'=>\$thread, 'maxgenethread=i'=>\$maxgenethread, 'polishgene'=>\$polishgene, 'xreffile=s'=>\$xreffile, 'convertarg=s'=>\$convertarg, 'codingarg=s'=>\$codingarg) or pod2usage ();
...
sub backConvertVCF {
	...
	my @nextrecord;
	my (@field, @prefield);
    #EI my ($anno_string, $pre_anno_string);
	my ($anno_string, $pre_anno_string, $variant_type_string, $variant_set);  #EI
	...
	while (<IN>) {
		...
		if (m/^##/) {
			...
			for my $i (0 .. @name-1) {
                ...
                #EI } elsif ($name[$i] =~ m/^(1000g\d+|esp\d+|cg\d+|popfreq|nci\d+)/) {
				} elsif ($name[$i] =~ m/^(1000g\d+|esp\d+|cg\d+|popfreq|nci\d+|ExAC\w+|VC\w+)/) {  #include ExAC and the VC batch frequency tables EI
			...
			print OUT qq{##INFO=<ID=VARIANT_TYPE,Number=.,Type=String,Description="VARIANT_TYPE annotation provided by ANNOVAR_EDDIE">\n};  ## setup new INFO line for VARIANT TYPE field EI
	...
	while (<MANNO>) {
		s/[\r\n]+$//;
		my $anno_string;
		my $variant_type_string;  #clear the VARIANT_TYPE field EI
		@field = split (/\t/, $_);
		...
		for my $i (5 .. @name-2) {		#these are all the annotation columns (starting from 6th column to the last one, which is Otherinfo which will not be written)
            ...
            if ($name[$i] =~ m/^(Func.refGene|ExonicFunc.refGene)$/) { ## If the field name is any of these then EI
			 	$variant_type_string .= "$field[$i]_";	# save the field value EI
            } ##EI
		}
		my $variant_set;  #clear the VARIANT_TYPE field EI
		if (index($variant_type_string,"splicing") != -1) { ## is this a splicing variant EI
			$anno_string .= ";VARIANT_TYPE=splicing"; ## append the VARIANT_TYPE field value splicing to the annotation string EI
		} elsif (index($variant_type_string,"ncRNA") != -1) { ## is this a ncRNA variant EI
			$anno_string .= ";VARIANT_TYPE=ncRNA"; ## append the VARIANT_TYPE field value ncRNA to the annotation string EI
		} elsif ((index($variant_type_string,"upstream") != -1) || (index($variant_type_string,"downstream") != -1)) { ## is this a stream variant EI
			$anno_string .= ";VARIANT_TYPE=upstream_downstream"; ## append the VARIANT_TYPE field value ncRNA to the annotation string EI
		} elsif ((index($variant_type_string,"UTR3") != -1) || (index($variant_type_string,"UTR5") != -1)) { ## is this a stream variant EI
			$anno_string .= ";VARIANT_TYPE=UTR3_5"; ## append the VARIANT_TYPE field value ncRNA to the annotation string EI
		} else { # keep the VARIANT_TYPE 
			$variant_set=0; # save field EI
			$anno_string .= ";VARIANT_TYPE="; ## append the VARIANT_TYPE field to the annotation string EI
			if (index($variant_type_string,"exonic") != -1) { ## is this a exonic variant EI
				$anno_string .= "exonic_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1; } # save field EI
			if (index($variant_type_string,"stopgain") != -1) { ## is this a exonic variant EI
				$anno_string .= "stopgain_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1; } # save field EI
			if (index($variant_type_string,"stoploss") != -1) { ## is this a exonic variant EI
				$anno_string .= "stoploss_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1; } # save field EI
			if (index($variant_type_string,"nonsynonymous_SNV") != -1) { ## is this a exonic variant EI
				$anno_string .= "nonsynonymous_SNV_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1;  # save field EI
			} elsif (index($variant_type_string,"synonymous_SNV") != -1) { ## is this a exonic variant EI
				$anno_string .= "synonymous_SNV_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1; } # save field EI
#			if (index($variant_type_string,"frameshift_deletion") != -1) { ## is this a exonic variant EI
#				$anno_string .= "frameshift_deletion_"; ## append the VARIANT_TYPE field to the annotation string EI
#				$variant_set=1; } # save field EI
#			if (index($variant_type_string,"frameshift_insertion") != -1) { ## is this a exonic variant EI
#				$anno_string .= "frameshift_insertion_"; ## append the VARIANT_TYPE field to the annotation string EI
#				$variant_set=1; } # save field EI
			if (index($variant_type_string,"nonframeshift_deletion") != -1) { ## is this a exonic variant EI
				$anno_string .= "nonframeshift_deletion_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1;  # save field EI
			} elsif (index($variant_type_string,"frameshift_deletion") != -1) { ## is this a exonic variant EI
				$anno_string .= "frameshift_deletion_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1; } # save field EI
			if (index($variant_type_string,"nonframeshift_insertion") != -1) { ## is this a exonic variant EI
				$anno_string .= "nonframeshift_insertion_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1;  # save field EI
			} elsif (index($variant_type_string,"frameshift_insertion") != -1) { ## is this a exonic variant EI
				$anno_string .= "frameshift_insertion_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1; } # save field EI
			if (index($variant_type_string,"unknown") != -1) { ## is this a exonic variant EI
				$anno_string .= "unknown_"; ## append the VARIANT_TYPE field to the annotation string EI
				$variant_set=1; } # save field EI
			if ($variant_set==0) { # test field EI 
				$anno_string .="$variant_type_string";} ## append the VARIANT_TYPE field to the annotation string EI
		}  #EI
...
sub geneOperation {
    ...
    # mictro - not necessary?  Could not find a modified annotate_variation_ed.pl
    #$sc = "annotate_variation_ed.pl -geneanno -buildver $buildver -dbtype $protocol -outfile $tempfile.$protocol -exonsort $queryfile $dbloc"; #EI
...
sub regionOperation {
    ...
    # mictro - not necessary?  Could not find a modified annotate_variation_ed.pl
    #my $sc = "annotate_variation_ed.pl -regionanno -dbtype $dbtype -buildver $buildver -outfile $tempfile $queryfile $dbloc"; #EI
	$argument and $sc .= " $argument";
...
sub filterOperation {
    ...
    # mictro - not necessary?  Have not found a modified annotate_variation_ed.pl yet.
    #my $sc = "annotate_variation_ed.pl -filter -dbtype $dbtype -buildver $buildver -outfile $tempfile $queryfile $dbloc"; #EI
	$argument and $sc .= " $argument";
	if ($thread) {
		$sc .= " -thread $thread";
	}
