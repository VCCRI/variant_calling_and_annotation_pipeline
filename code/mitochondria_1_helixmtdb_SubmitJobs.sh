...
while read sample infile outdir; do
  ...
  outfile_mito="${outdir}"/"${infile_basename_prefix}".chromM.vcf
  outfile="${outdir}"/"${infile_basename_prefix}".chromM.helixmtdb
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo 'qsub -N' ${jobid} '-v infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',outfile_mito='"${outfile_mito}"',sw_and_refs='"${sw_and_refs}" 'mitochondria_1_helixmtdb.sh'
      qsub -N ${jobid} -v infile="${infile}",outdir="${outdir}",outfile="${outfile}",outfile_mito="${outfile_mito}",sw_and_refs="${sw_and_refs}" mitochondria_1_helixmtdb.sh # && \
      touch "${queue_file}" && \
      echo "${jobid} queued" $infile && \
      sleep 1
      num_queued=$((num_queued+1))

      #echo './mitochondria_1_helixmtdb.sh' $infile $outdir $outfile $outfile_mito $sw_and_refs
      #./mitochondria_1_helixmtdb.sh $infile $outdir $outfile $outfile_mito $sw_and_refs
