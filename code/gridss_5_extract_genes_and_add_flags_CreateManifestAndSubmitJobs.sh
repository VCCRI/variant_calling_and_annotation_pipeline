...
for infile in "${indir}"/*.CDSx.Gene.bndX.gCDSx.gGene.gBndX.EHRF.gSV.DGV.Dup.dbsc.TSS.pext.Idel.Ishort.gIshort.hot.compareSamples.compareLowQual.tsv; do
  ...
  family_or_sample="${array[0]}" # this is either the sample id or it is the AGHA family id if there is one
  ...
  infile_tsv=$infile
  outfile_basename_prefix="${family_and_samples}".gridss
