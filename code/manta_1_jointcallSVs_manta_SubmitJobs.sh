...
while read sample infile outdir outdir2 outfile; do
  ...
  outfile="${outdir}"/"${sample}".markDup.setTags.bqsr.manta
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo 'qsub -N' ${jobid} '-v sample='"${sample}"',infile='"${infile}"',outdir='"${outdir}"',outdir2='"${outdir2}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}" 'manta_1_jointcallSVs_manta.sh'
      qsub -N ${jobid} -v sample="${sample}",infile="${infile}",outdir="${outdir}",outdir2="${outdir2}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" manta_1_jointcallSVs_manta.sh && \
      touch "${queue_file}" && \
      echo "${jobid} queued" $infile && \
      sleep 1
      num_queued=$((num_queued+1))

      #./manta_1_jointcallSVs_manta.sh $sample $infile $outdir $outdir2 $outfile $sw_and_refs
