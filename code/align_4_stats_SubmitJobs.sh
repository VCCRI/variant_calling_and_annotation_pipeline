...
while read sample infile outdir; do
  ...
  jobid=sta"${sample}"
  outfile="${outdir}"/"${infile_basename_prefix}".Picard_CollectWgsMetrics.txt
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      qsub -N ${jobid} -v infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" align_4_stats.sh && \
      touch "${queue_file}" && \
