...
while read sample infile_prefix infile_suffix outdir cohort; do # only genes required by cohort will be output
  ...
  outfile="${outdir}"/"${sample}".audacity_homozygosity_output.txt
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      #echo 'qsub -N' ${jobid} '-v sample='"${sample}"',infile_prefix='"${infile_prefix}"',infile_suffix='"${infile_suffix}"',outdir='"${outdir}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}"',cohort='"${cohort}" 'audacity_homozygosity_2_concat_audacity_sample.sh'
      #qsub -N ${jobid} -v sample="${sample}",infile_prefix="${infile_prefix}",infile_suffix="${infile_suffix}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}",cohort="${cohort}" audacity_homozygosity_2_concat_audacity_sample.sh # && \
      #touch "${queue_file}" && \
      ...
      echo './audacity_homozygosity_2_concat_audacity_sample.sh' $sample $infile_prefix $infile_suffix $outdir $outfile $sw_and_refs $cohort
      ./audacity_homozygosity_2_concat_audacity_sample.sh $sample $infile_prefix $infile_suffix $outdir $outfile $sw_and_refs $cohort
