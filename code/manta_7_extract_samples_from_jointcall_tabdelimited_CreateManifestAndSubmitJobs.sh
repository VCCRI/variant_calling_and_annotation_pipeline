...
# FAMILY_FOUR_SAMPLE_ONE_SAMPLE_TWO_SAMPLE_THREE.manta.CHD_all2022jun_955_genes.vpot_final_output_file.txt
...
while IFS= read -r inline; do
  ...
  if [[ $family != '.' ]]; then
    ...
    while [[ $i -lt $arr_len ]] ; do
      ...
      infile=$(ls -1 "${indir}"/"${family}"*.manta.*_genes.vpot_final_output_file.txt | head -n 1)
