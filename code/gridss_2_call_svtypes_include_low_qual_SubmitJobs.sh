...
while read sample infile outdir outfile_basename; do
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      qsub -N ${jobid} -v sample="${sample}",infile="${infile}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" gridss_2_call_svtypes_include_low_qual.sh # && \
      touch "${queue_file}" && \
      echo "${jobid} queued" $infile && \
      sleep 1
      num_queued=$((num_queued+1))

      #echo './gridss_2_call_svtypes_include_low_qual.sh' $sample $infile $outfile $sw_and_refs
      #./gridss_2_call_svtypes_include_low_qual.sh $sample $infile $outfile $sw_and_refs
