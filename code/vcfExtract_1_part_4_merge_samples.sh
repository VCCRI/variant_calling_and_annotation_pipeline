...
infile_basename_suffix=.gatk_platypus_mnp.AllChrom.annovar_vep.possiblyPathogenic.vpot_final_output_file.vpot_gt_0.txt
outfile_basename_suffix=_"${batch}"__gatk_platypus_mnp.AllChrom.annovar_vep.possiblyPathogenic.vpot_final_output_file.vpot_gt_0.txt
tmphdr='temp_hdr.txt'
...
for cohort in "${cohorts[@]}"; do
  ...
  cat $string_of_files | grep -P -v '^Rank|^chrom' | sort -k1nr -k2nr | cat $tmphdr2 - > $outfile
  echo ''
