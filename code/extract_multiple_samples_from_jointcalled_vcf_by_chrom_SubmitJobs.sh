...
while read infile outdir outfile_basename; do
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo 'qsub -N' ${jobid} '-v infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}" 'extract_multiple_samples_from_jointcalled_vcf_by_chrom.sh'
      qsub -N ${jobid} -v infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" extract_multiple_samples_from_jointcalled_vcf_by_chrom.sh # && \
      touch "${queue_file}" && \
      ...
      #./extract_multiple_samples_from_jointcalled_vcf_by_chrom.sh "${infile}" "${outdir}" "${outfile}" "${sw_and_refs}"
