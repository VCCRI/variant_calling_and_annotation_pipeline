...
rand_num=$RANDOM
tmpdir="${PBS_JOBFS}"/tmp_"${rand_num}"
mkdir -p "${tmpdir}"
...
if [[ $vep_impact_col -gt -1 ]]; then
  echo 'awk -v vep_impact_col='"$vep_impact_col" 'BEGIN {FS="\t";OFS="\t"} {if (('$vep_impact_col' ~ /HIGH/) || ($0 ~ /transcript_ablation/) || ($0 ~ /splice_acceptor_variant/) || ($0 ~ /splice_donor_variant/) || ($0 ~ /stop_gained/) || ($0 ~ /frameshift_variant/) || ($0 ~ /stop_lost/) || ($0 ~ /start_lost/) || ($0 ~ /transcript_amplification/) || ($0 ~ /p\.M1/) || ($2 != "0")) {print $0}}' "${vpot_output_file_adjusted_scores}" '| uniq >' "${vpot_output_file_vpot_gt_0}"
  awk -v vep_impact_col="$vep_impact_col" 'BEGIN {FS="\t";OFS="\t"} {if (($vep_impact_col ~ /HIGH/) || ($0 ~ /transcript_ablation/) || ($0 ~ /splice_acceptor_variant/) || ($0 ~ /splice_donor_variant/) || ($0 ~ /stop_gained/) || ($0 ~ /frameshift_variant/) || ($0 ~ /stop_lost/) || ($0 ~ /start_lost/) || ($0 ~ /transcript_amplification/) || ($0 ~ /p\.M1/) || ($2 != "0")) {print $0}}' "${vpot_output_file_adjusted_scores}" | uniq > "${vpot_output_file_vpot_gt_0}"
else
  echo 'awk' 'BEGIN {FS="\t";OFS="\t"} {if (($0 ~ /transcript_ablation/) || ($0 ~ /splice_acceptor_variant/) || ($0 ~ /splice_donor_variant/) || ($0 ~ /stop_gained/) || ($0 ~ /frameshift_variant/) || ($0 ~ /stop_lost/) || ($0 ~ /start_lost/) || ($0 ~ /transcript_amplification/) || ($0 ~ /p\.M1/) || ($2 != "0")) {print $0}}' "${vpot_output_file_adjusted_scores}" '| uniq >' "${vpot_output_file_vpot_gt_0}"
  awk 'BEGIN {FS="\t";OFS="\t"} {if (($0 ~ /transcript_ablation/) || ($0 ~ /splice_acceptor_variant/) || ($0 ~ /splice_donor_variant/) || ($0 ~ /stop_gained/) || ($0 ~ /frameshift_variant/) || ($0 ~ /stop_lost/) || ($0 ~ /start_lost/) || ($0 ~ /transcript_amplification/) || ($0 ~ /p\.M1/) || ($2 != "0")) {print $0}}' "${vpot_output_file_adjusted_scores}" | uniq > "${vpot_output_file_vpot_gt_0}"
fi
echo ''
