...
while read sample infile outdir outfile_basename; do
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo 'qsub -N' ${jobid} '-v sample='"${sample}"',infile='"${infile}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}" 'gridss_2_call_svtypes_SubmitJobs.sh'
      qsub -N ${jobid} -v sample="${sample}",infile="${infile}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" gridss_2_call_svtypes_SubmitJobs.sh && \
      touch "${queue_file}" && \
      echo "${jobid} queued" $infile && \
      sleep 1
      num_queued=$((num_queued+1))

      #./gridss_2_call_svtypes_SubmitJobs.sh $sample $infile $outfile $sw_and_refs
