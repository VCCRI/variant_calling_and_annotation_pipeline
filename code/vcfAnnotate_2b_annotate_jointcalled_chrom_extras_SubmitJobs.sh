...
while read batch infile outdir; do
  ...
  infile_basename_prefix="${infile_basename_prefix%.removeAltAsterisk.annovar_clinvarDATE."${genome_version}"_multianno.rmvSomeAnnovar.vep_reformatted}".annovar_clinvarDATE.vep_reformatted
  ...
  outfile="${outdir}"/"${infile_basename_prefix}".extras.vcf
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo 'qsub -N' ${jobid} '-v infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',chrom='"${chrom}"',sw_and_refs='"${sw_and_refs}" 'vcfAnnotate_2b_annotate_jointcalled_chrom_extras.sh'
      qsub -N ${jobid} -v infile="${infile}",outdir="${outdir}",outfile="${outfile}",chrom="${chrom}",sw_and_refs="${sw_and_refs}" vcfAnnotate_2b_annotate_jointcalled_chrom_extras.sh && \
      touch "${queue_file}" && \
