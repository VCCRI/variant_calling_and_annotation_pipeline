...
while read sample tool param_file infile outdir out_prefix_basename; do
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      #qsub -N ${jobid} -v sample="${sample}",tool="${tool}",param_file="${param_file}",infile="${infile}",outdir="${outdir}",out_prefix="${out_prefix}",sw_and_refs="${sw_and_refs}" call_vpot_sv_priority.sh # && \
      #touch "${queue_file}" && \
      ...
      ./call_vpot_sv_priority.sh "${sample}" "${tool}" "${param_file}" "${infile}" "${outdir}" "${out_prefix}" "${sw_and_refs}"
    fi
  #fi
