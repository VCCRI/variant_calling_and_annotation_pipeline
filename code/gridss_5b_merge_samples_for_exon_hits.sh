...
indir=/my/directory/variant_calling_and_annotation_pipeline/working_directory/gridss_annotate
             ...
             infile_basename_suffix_CHD_singles=.gridss.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_CHD_singles=_"${batch}"__gridss.extract_exons_hit_and_add_flags.txt

             infile_basename_suffix_CHD_family=.gridss.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_CHD_family=_"${batch}"__gridss.extract_exons_hit_and_add_flags.txt

             infile_basename_suffix_DCM_singles=.gridss.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_DCM_singles=_"${batch}"__gridss.extract_exons_hit_and_add_flags.txt

             infile_basename_suffix_DCM_family=.gridss.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_DCM_family=_"${batch}"__gridss.extract_exons_hit_and_add_flags.txt
  ...
  for singles_or_family in "singles" "family"; do
    ...
    if [[ $infile_basename_suffix != "" ]]; then
      ...
      if [[ $one_file != '' ]]; then # some files were found in this cohort and singles_or_family, so output a merged file of those files
        ...
        tmphdr="${tmpdir}"/gridss_7_merge_samples_for_exon_hits.temp_gridss_hdr.txt
        tmphdr2="${tmpdir}"/gridss_7_merge_samples_for_exon_hits.temp_gridss_hdr2.txt
