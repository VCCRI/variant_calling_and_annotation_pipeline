...
while read sample infile_prefix infile_suffix outdir cohort; do # only genes required by cohort will be output
  ...
  outfile="${outdir}"/"${infile_prefix_basename}".AllChrom"${infile_suffix_prefix}".extracts.tsv
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo 'qsub -N' ${jobid} '-v sample='"${sample}"',infile_prefix='"${infile_prefix}"',infile_suffix='"${infile_suffix}"',outdir='"${outdir}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}"',cohort='"${cohort}" 'vcfExtract_1_part_1_extract_pathogenic_and_add_flags.sh'
      qsub -N ${jobid} -v sample="${sample}",infile_prefix="${infile_prefix}",infile_suffix="${infile_suffix}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}",cohort="${cohort}" vcfExtract_1_part_1_extract_pathogenic_and_add_flags.sh # && \
      touch "${queue_file}" && \
      echo "${jobid} queued" $infile # && \
      sleep 1
      num_queued=$((num_queued+1))

      #./vcfExtract_1_part_1_extract_pathogenic_and_add_flags.sh $sample $infile_prefix $infile_suffix $outdir $outfile $sw_and_refs $cohort
