...
outdir=/my/directory/variant_calling_and_annotation_pipeline/working_directory/bam_qc
...
for infile_done in "${indir}"/*.markDup.setTags.bqsr.bam.done; do
  ...
  infile_basename=$(basename $infile_done)
...
./bamQC_2_picard_stats_SubmitJobs.sh $out_manifest
