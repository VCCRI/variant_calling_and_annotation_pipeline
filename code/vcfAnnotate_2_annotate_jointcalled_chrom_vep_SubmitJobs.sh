...
while read batch infile outdir; do
  ...
  outfile="${outdir}"/"${infile_basename_prefix}".vep.vcf
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo 'qsub -N' ${jobid} '-v infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',chrom='"${chrom}"',sw_and_refs='"${sw_and_refs}" 'vcfAnnotate_2_annotate_jointcalled_chrom_vep.sh'
      qsub -N ${jobid} -v infile="${infile}",outdir="${outdir}",outfile="${outfile}",chrom="${chrom}",sw_and_refs="${sw_and_refs}" vcfAnnotate_2_annotate_jointcalled_chrom_vep.sh && \
      touch "${queue_file}" && \
