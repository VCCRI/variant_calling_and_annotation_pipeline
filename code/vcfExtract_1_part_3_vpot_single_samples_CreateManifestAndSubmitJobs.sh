...
for infile in "${indir}"/*.gatk_vqsr_and_platypus_mnp.AllChrom.annovar_clinvarDATE.vep_reformatted.extras.homrun.extracts.*_possiblyPathogenic.tsv; do
  ...
  param_file='/my/directory/variant_calling_and_annotation_pipeline/code/default_VPOT_MAF_0p01_PPF_for_CHD_names.txt'
  if [[ $cohort == "DCM" ]]; then
    param_file='/my/directory/variant_calling_and_annotation_pipeline/code/default_VPOT_MAF_0p01_PPF_for_DCM_names.txt'
  fi

  out_prefix_basename="${sample}.gatk_platypus_mnp.AllChrom.annovar_vep.possiblyPathogenic.vpot"
...
./call_vpot_vcf_tsv_single_samples_for_pathogenic_SubmitJobs.sh $out_manifest
