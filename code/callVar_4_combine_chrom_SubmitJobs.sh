...
while read sample infile_prefix infile_suffix outfile; do
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      qsub -N ${jobid} -v infile_prefix="${infile_prefix}",infile_suffix="${infile_suffix}",outdir="${outdir}",outfile="${outfile}",chroms="${chroms}",sw_and_refs="${sw_and_refs}" callVar_4_combine_chrom.sh && \
      touch "${queue_file}" && \
      echo "${jobid} queued" $infile_prefix $infile_suffix && \
      sleep 1
