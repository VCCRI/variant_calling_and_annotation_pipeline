...
outfile_basename=all.commands_to_generate_igv_screenshots.annovar_stoploss_stopgain.txt
cohorts=(CHD DCM)
...
for cohort in "${cohorts[@]}"; do
  ...
  infile=/my/directory/variant_calling_and_annotation_pipeline/working_directory/vcf_chroms_MNP_annotate_samples_extracts/"${cohort}"_"${batch}"__gatk_platypus_mnp.AllChrom.annovar_vep.stopLossGain.vpot_final_output_file.vpot_gt_0.txt
  ...
  for this_col in "${array[@]}"; do
    ...
    if [[ $this_col == "INFO_AN" ]]; then
      AN_col=$i
    fi
    if [[ $this_col == "INFO_FR" ]]; then
      FR_col=$i
    fi
  done
  ...
  grep -v '^Rank' $infile | awk -v sample_col="$sample_col" -v gene_col="$gene_col" -v AN_col="$AN_col" -v FR_col="$FR_col" 'BEGIN {FS="\t";OFS="\t"} {if ((($AN_col!=".")&&($AN_col<=1)) || (($FR_col!=".")&&($FR_col<0.1))) {print $sample_col, $3, $4, $5, $gene_col}}' | sort | uniq > $tmp_infile || true
  current_sample=""
