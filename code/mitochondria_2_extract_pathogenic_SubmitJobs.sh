...
while read sample infile outdir; do
  ...
  infile_basename_prefix="${infile_basename%.tsv}"
  infile_basename_prefix="${infile_basename_prefix%.txt}"

  outfile="${outdir}"/"${infile_basename_prefix}".possibly_pathogenic.tsv
  ...
  log_file="${outfile}.log"
    ...
    #  echo "${jobid} was terminated" $term_file
      ...
      #echo 'qsub -N' ${jobid} '-v sample="${sample}",infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}" 'mitochondria_2_extract_pathogenic.sh'
      #qsub -N ${jobid} -v sample="${sample}",infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" mitochondria_2_extract_pathogenic.sh # && \
      #touch "${queue_file}" && \
      #echo "${jobid} queued" $infile && \
      #sleep 1
      #num_queued=$((num_queued+1))

      echo './mitochondria_2_extract_pathogenic.sh' $sample $infile $outdir $outfile $sw_and_refs
      ./mitochondria_2_extract_pathogenic.sh $sample $infile $outdir $outfile $sw_and_refs
