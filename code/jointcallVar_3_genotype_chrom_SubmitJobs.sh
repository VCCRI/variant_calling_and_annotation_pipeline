...
while read chrom infile outdir; do
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo 'qsub -N' ${jobid} '-v infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',chrom='"${chrom}"',sw_and_refs='"${sw_and_refs}" 'jointcallVar_3_genotype_chrom.sh'
      qsub -N ${jobid} -v infile="${infile}",outdir="${outdir}",outfile="${outfile}",chrom="${chrom}",sw_and_refs="${sw_and_refs}" jointcallVar_3_genotype_chrom.sh && \
      touch "${queue_file}" && \
