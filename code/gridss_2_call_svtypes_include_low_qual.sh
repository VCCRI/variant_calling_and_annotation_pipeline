...
tmpdir="${PBS_JOBFS}"/tmp/"${sample}"__"${infile_basename}"__"${outfile_basename}"
mkdir -p "${tmpdir}"
...
##################################################
# Do not filter out low quality SV calls
##################################################
...
echo ''
echo 'cat' "${gridss_vcf_file}" '| python' "${SVannotation_software_directory}"'/programs/add_tranche2_and_filter_gridss_vcf.py -max_BND_length_for_no_split_reads 100 -vaf_version 2 >' "${tmpdir}/${sample}_gridss_BND_filtered1.vcf"
echo ''
cat "${gridss_vcf_file}" | python "${SVannotation_software_directory}"/programs/add_tranche2_and_filter_gridss_vcf.py -max_BND_length_for_no_split_reads 100 -vaf_version 2 \
...
grep '^#' "${tmpdir}/${sample}_gridss_BND_filtered1.vcf" > "${tmpdir}/${sample}_gridss_hdr.vcf"
grep -v '^#' "${tmpdir}/${sample}_gridss_BND_filtered1.vcf" | sort -t$'\t' -k 3,3 > "${tmpdir}/${sample}_gridss_nohdr_sortedByEvent.txt" || true
...
##################################################
# Remove some of the GRIDSS output fields
##################################################
...
# #Chr	Start	End	Ref	Alt	Qual	END	SVLEN	SVTYPE	TRANCHE	TRANCHE2	BNDVAF	CIEND_1	CIEND_2	CIPOS_1	CIPOS_2	INSSEQ	VAF	gridssAS	gridssASQ	gridssASRP	gridssASRR	gridssBA	gridssBAQ	gridssBEID	gridssBQ	gridssBSC	gridssBSCQ	gridssBUM	gridssBUMQ	gridssCAS	gridssCASQ	gridssCIEND	gridssCIPOS	gridssCIRPOS	gridssCQ	gridssFILTER	gridssHOMLEN	gridssHOMSEQ	gridssIC	gridssIHOMPOS	gridssIQ	gridssQUAL	gridssRAS	gridssRASQ	gridssREF	gridssREFPAIR	gridssRP	gridssRPQ	gridssRSI	gridssSI	gridssSR	gridssSRQ	gridssVAF	INVBND1BDR3INS	INVBND1BDR5INS	INVBND1DEL	INVBND1OVERLAP	INVBND2BDR3INS	INVBND2BDR5INS	INVBND2DEL	INVBND2OVERLAP	sample_id
