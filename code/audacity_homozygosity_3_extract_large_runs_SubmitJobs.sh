...
while read sample infile outdir; do # only genes required by cohort will be output
  ...
  outfile="${outdir}"/"${infile_basename%.txt}"
  outfile="${outfile%.tsv}".large_homozygosity_runs_only.txt
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo "${jobid} was terminated" $term_file
        ...
        #echo 'qsub -N' ${jobid} '-v sample='"${sample}"',infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}" audacity_homozygosity_3_extract_large_runs.sh'
        #qsub -N ${jobid} -v sample="${sample}",infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" audacity_homozygosity_3_extract_large_runs.sh # && \
        #touch "${queue_file}" && \
        #echo "${jobid} queued" $infile # && \
        #sleep 1
        #num_queued=$((num_queued+1))

        echo './audacity_homozygosity_3_extract_large_runs.sh' $sample $infile $outdir $outfile $sw_and_refs
        ./audacity_homozygosity_3_extract_large_runs.sh $sample $infile $outdir $outfile $sw_and_refs
