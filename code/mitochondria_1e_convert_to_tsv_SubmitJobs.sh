...
while read sample infile outdir; do
  ...
  outfile="${outdir}"/"${infile_basename_prefix}".extramito.tsv
  ...
  log_file="${outfile}.log"
    ...
    #  echo "${jobid} was terminated"
      ...
      #echo 'qsub -N' ${jobid} '-v infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}" 'mitochondria_1e_convert_to_tsv.sh'
      #qsub -N ${jobid} -v infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" mitochondria_1e_convert_to_tsv.sh # && \
      #touch "${queue_file}" && \
      #echo "${jobid} queued" $infile && \
      #sleep 1
      #num_queued=$((num_queued+1))

      echo './mitochondria_1e_convert_to_tsv.sh' $infile $outdir $outfile $sw_and_refs
      ./mitochondria_1e_convert_to_tsv.sh $infile $outdir $outfile $sw_and_refs
