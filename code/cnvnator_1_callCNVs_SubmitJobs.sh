...
while read sample infile outdir; do
  ...
  jobid=cnv"${sample}"
  outfile="${outdir}"/"${infile_basename_prefix}".cnvnator.txt
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      qsub -N ${jobid} -v infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" cnvnator_1_callCNVs.sh && \
      touch "${queue_file}" && \
