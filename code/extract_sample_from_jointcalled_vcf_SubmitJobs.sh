...
while read sample infile outdir outfile_basename; do
    ...
    log_file="${outfile}.log"
      ...
      if [ -e "${queue_file}" ]; then
        ...
        echo 'qsub -N' ${jobid} '-v sample='"${sample}"',infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}" 'extract_sample_from_jointcalled_vcf.sh'
        qsub -N ${jobid} -v sample="${sample}",infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" extract_sample_from_jointcalled_vcf.sh # && \
        touch "${queue_file}" && \
        ...
        #touch "${queue_file}"
        #./extract_sample_from_jointcalled_vcf.sh "${sample}" "${infile}" "${outdir}" "${outfile}" "${sw_and_refs}"
