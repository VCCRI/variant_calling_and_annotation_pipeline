...
while read sample infile outdir; do
  ...
  jobid=grd"${sample}"
  outfile="${outdir}"/"${infile_basename_prefix}".gridss.vcf.gz
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      qsub -N ${jobid} -v sample="${sample}",infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" gridss_1_callSVs_gridss.sh && \
      touch "${queue_file}" && \
