...
indir=/my/directory/variant_calling_and_annotation_pipeline/working_directory/cnvnator_annotate
          ...
          infile_basename_suffix_CHD_singles=.cnvnator.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_CHD_singles=_"${batch}"__cnvnator.extract_exons_hit_and_add_flags.txt

          infile_basename_suffix_DCM_singles=.cnvnator.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_DCM_singles=_"${batch}"__cnvnator.extract_exons_hit_and_add_flags.txt
  ...
  for singles_or_family in "singles"; do # cnvnator does not do joint calls, so there are no merged family results like there is for gridss and manta
    ...
    if [[ $infile_basename_suffix != "" ]]; then
      ...
      if [[ $one_file != '' ]]; then # some files were found in this cohort and singles_or_family, so output a merged file of those files
        ...
        tmphdr="${tmpdir}"/cnvnator_7_merge_samples_for_exon_hits.temp_cnvnator_hdr.txt
        tmphdr2="${tmpdir}"/cnvnator_7_merge_samples_for_exon_hits.temp_cnvnator_hdr2.txt
