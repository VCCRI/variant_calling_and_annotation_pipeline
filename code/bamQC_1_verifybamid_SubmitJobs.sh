...
while read sample infile outdir; do
  ...
  jobid=ver"${sample}"
  outfile="${outdir}"/"${infile_basename_prefix}".verifybamid2_output.txt
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      echo 'qsub -N' ${jobid} '-v sample='"${sample}"',infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}" 'bamQC_1_verifybamid.sh'
      qsub -N ${jobid} -v sample="${sample}",infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" bamQC_1_verifybamid.sh && \
      touch "${queue_file}" && \
