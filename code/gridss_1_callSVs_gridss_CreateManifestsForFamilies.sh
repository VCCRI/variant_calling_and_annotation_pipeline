  ...
  if [[ $Family != "Family" ]]; then
    if [[ $Family != $Subject ]]; then
      if [[ $Family != $prev_family ]]; then
        out_manifest=./manifests/test_manifest.gridss."${Family}".txt
        :>$out_manifest
        echo './gridss_1_jointcallSVs_gridss_SubmitJobs.sh' $out_manifest
        prev_family=$Family
      fi
      sample=$Subject
      infile="${indir}"/"${sample}".markDup.setTags.bqsr.bam
      echo -e "${sample}\t${Family}\t${infile}\t${outdir}" >> "${out_manifest}"
    fi
  fi
done < $pedigree_file
echo ''
