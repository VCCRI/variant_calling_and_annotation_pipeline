...
while read sample infile outdir; do
  ...
  outfile="${outdir}"/"${infile_basename_prefix}".mitomap
  ...
  log_file="${outfile}.log"
    ...
    #  echo "${jobid} was terminated" $term_file
      ...
      #echo 'qsub -N' ${jobid} '-v infile='"${infile}"',outdir='"${outdir}"',outfile='"${outfile}"',sw_and_refs='"${sw_and_refs}" 'mitochondria_1d_mitomap.sh'
      #qsub -N ${jobid} -v infile="${infile}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" mitochondria_1d_mitomap.sh # && \
      #touch "${queue_file}" && \
      #echo "${jobid} queued" $infile && \
      #sleep 1
      #num_queued=$((num_queued+1))

      echo './mitochondria_1d_mitomap.sh' $infile $outdir $outfile $sw_and_refs
      ./mitochondria_1d_mitomap.sh $infile $outdir $outfile $sw_and_refs
