...
indir=/my/directory/variant_calling_and_annotation_pipeline/working_directory/manta_annotate
             ...
             infile_basename_suffix_CHD_singles=.manta.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_CHD_singles=_"${batch}"__manta.extract_exons_hit_and_add_flags.txt

             infile_basename_suffix_CHD_family=.manta.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_CHD_family=_"${batch}"__manta.extract_exons_hit_and_add_flags.txt

             infile_basename_suffix_DCM_singles=.manta.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_DCM_singles=_"${batch}"__manta.extract_exons_hit_and_add_flags.txt

             infile_basename_suffix_DCM_family=.manta.extract_exons_hit_and_add_flags.txt
outfile_basename_suffix_DCM_family=_"${batch}"__manta.extract_exons_hit_and_add_flags.txt
  ...
  for singles_or_family in "singles" "family"; do
    ...
    if [[ $infile_basename_suffix != "" ]]; then
      ...
      if [[ $one_file != '' ]]; then # some files were found in this cohort and singles_or_family, so output a merged file of those files
        ...
        tmphdr="${tmpdir}"/manta_7_merge_samples_for_exon_hits.temp_manta_hdr.txt
        tmphdr2="${tmpdir}"/manta_7_merge_samples_for_exons_hits.temp_manta_hdr2.txt
