...
while read sample infile_prefix infile_suffix outdir outfile; do
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      qsub -N ${jobid} -v sample="${sample}",infile_prefix="${infile_prefix}",infile_suffix="${infile_suffix}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" concat_vcf_gz_chroms.sh # && \
      #touch "${queue_file}" && \
      ...
      #touch "${queue_file}"
      #./concat_vcf_gz_chroms.sh "${sample}" "${infile_prefix}" "${infile_suffix}" "${outdir}" "${outfile}" "${sw_and_refs}"
