...
while read chrom outdir infile_list; do

  outfile="${outdir}"/"${chrom}"
  ...
  log_file="${outfile}.log"
    ...
    if [ -e "${queue_file}" ]; then
      ...
      qsub -N ${jobid} -v chrom="${chrom}",infile_list="${infile_list}",outdir="${outdir}",outfile="${outfile}",sw_and_refs="${sw_and_refs}" jointcallVar_2_GenomicsDBImport_chrom.sh && \
      touch "${queue_file}" && \
